...
export async function run(canvas, { pausedRef, options = {} } = {}) {
  ...
  const TEXTURE_CONTRAST = options.textureContrast   ?? 0.9; // subtle
  ...
  // Assets (Mars)
  const TEX_ALBEDO = options.albedoTexture ?? '/interactive/moon/moon_texture.jpg';
  const TEX_STARS  = options.starsTexture  ?? '/interactive/moon/stars_texture.jpg';
