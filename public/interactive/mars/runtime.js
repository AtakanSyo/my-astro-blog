...
export async function run(canvas, { pausedRef, options = {} } = {}) {
  ...
  const TEXTURE_SAT      = options.textureSaturation ?? 1.00; // 1.0 = original
  const TEXTURE_CONTRAST = options.textureContrast   ?? 0.8; // subtle

  const SUN_INTENSITY     = options.lightIntensity ?? 3.0;   // DirectionalLight power
  const AMBIENT_INTENSITY = options.ambientIntensity ?? 0.08; // Ambient fill
