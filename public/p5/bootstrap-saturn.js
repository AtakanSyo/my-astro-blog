// /public/p5/bootstrap-saturn.js
// Bootstraps the Saturn rings sim. External file so MDX won't parse it.

(function () {
  var startBtn  = document.getElementById('start-saturn-sim');
  ...
  function syncOutputs() {
    if (outRot)  outRot.textContent  = fmt(val(speedIn, 0.35), 2);
    if (outTilt) outTilt.textContent = fmt(val(tiltIn, 18), 0);
  }
  ...
  if (startBtn) startBtn.addEventListener('click', function () {
    ...
    loadP5().then(function () {
      return import('/p5/saturn.js');
    }).then(function (mod) {
      // Mount into #saturn-sim
      new window.p5(mod.saturnSketch, 'saturn-sim');
    });
  });
